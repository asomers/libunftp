...
mod list {
    ...
    /// test the exact format of the output for regular files
    #[cfg(unix)]
    #[rstest]
    #[awt]
    #[tokio::test]
    ...
    #[awt]
    #[tokio::test]
    async fn symlink(#[case] #[future] harness: Harness, #[case] absolute: bool) {
        use regex::Regex;
        ...
        ftp_stream.login("hoi", "jij").await.unwrap();
        let list = ftp_stream.list(None).await.unwrap();
        let pat = format!("^lrwxr-xr-x\\s+{link_count}\\s+{uid}\\s+{gid}\\s+{size}.*link -> {target}");
        let re = Regex::new(&pat).unwrap();
        for entry in list {
